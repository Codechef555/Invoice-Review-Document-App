...
export function UploadStep({ file, error, onChoose, onProcess, onBack }: UploadStepProps) {
  ...
  function choose(next?: File) {
    ...
    if (next.size > maxBytes) {
      setLocalError('This app accepts files up to 4 MB.')
      return
    }
    setLocalError(null)
    onChoose(next)
      ...
      <div className="mb-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Check the preview first. The file is sent for pipeline analysis when you confirm.
          </p>
        </div>
      </div>
      ...
      <div className={`grid gap-6 ${file ? 'lg:grid-cols-[1fr_360px]' : ''}`}>
        {file ? (
          ...
          <button
            ...
            className="flex min-h-[400px] w-full cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-zinc-300 bg-white p-10 text-center transition hover:border-zinc-500 hover:bg-zinc-50"
          >
          ...
          <Card className="h-fit">
            ...
            <CardContent>
              ...
              <p className="mt-5 border-t border-zinc-100 pt-5 text-sm leading-6 text-zinc-500">
                The app classifies the document, extracts fields, validates VAT & business policy,
                and suggests a General Ledger account.
              </p>
